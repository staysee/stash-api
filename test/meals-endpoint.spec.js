...
describe(' Meals Endpoints', () => {
      ...
      beforeEach('insert meals', () => db
        ...
        .then(() => db
          ...
          .then(() => db
            .into('meals')
            .insert(testMeals))));
      it('responds with 200 and all of the meals', () => supertest(app)
        .get('/api/meals')
        .set('Authorization', token)
  ...
  describe('GET /api/meals/user', () => {
    ...
    context('Given user has meals', () => {
      const mealsOutput = helpers.makeMealsObject();
      beforeEach('insert user, recipes, meals', () => db
        .into('users')
        .insert(testUsers)
        .then(() => db
          .into('recipes')
      ...
      it('responds with 200 and all the user meals', () => supertest(app)
        .get('/api/meals/user')
        .set('Authorization', token)
        .expect(200)
        .expect((res) => {
  ...
  describe('POST /api/meals', () => {
    before('insert users and recipes', () => db
      .into('users')
      .insert(testUsers)
      .then(() => db
        .into('recipes')
