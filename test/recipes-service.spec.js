...
describe('Recipes service object', () => {
  let db;
  ...
  context('Given \'recipes\' has data', () => {
    beforeEach('insert recipes', () => db
      ...
      .then(() => db
        .into('recipes')
        .insert(testRecipes)));
    it('getAllRecipes() resolves all recipes from \'recipes\' table', () => RecipesService
      .getAllRecipes(db)
      .then((actual) => {
  ...
  context('Given \'recipes\' has no data', () => {
    beforeEach(() => db
      .into('users')
      .insert(testUsers));
    it('getAllRecipes() resolves an empty array', () => RecipesService.getAllRecipes(db)
      .then((actual) => {
        expect(actual).to.eql([]);
      }));
